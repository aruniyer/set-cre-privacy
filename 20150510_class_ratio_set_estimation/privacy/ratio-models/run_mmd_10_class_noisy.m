function run_mmd_10_class_noisy(dataset, outfile)
...
for seed = 1:10
    for test_theta1 = 0.1:0.1:0.9
        ...
        for seedi = 1:1
            ...
            printToFile(fid, dataset, 'MMD', seed, 'laplace', split.train_bag_prop, test_set_proportion', mmd_props);
            ...
            kernel = Ks{mmd_parameters(seed, 1)};
            mmd_props = MMD(kernel, split, []);
            printToFile(fid, dataset, 'MMD', seed, 'gaussian', split.train_bag_prop, test_set_proportion', mmd_props);
            ...
            kernel = Ks{mmd_parameters(seed, 1)};
            mmd_props = MMD(kernel, split, []);
            printToFile(fid, dataset, 'MMD', seed, 'dirichlet', split.train_bag_prop, test_set_proportion', mmd_props);
            ...
            kernel = Ks{mmd_parameters(seed, 1)};
            mmd_props = MMD(kernel, split, []);
            printToFile(fid, dataset, 'MMD', seed, 'ashwin', split.train_bag_prop, test_set_proportion', mmd_props);
        end
    end
end
fclose(fid);
