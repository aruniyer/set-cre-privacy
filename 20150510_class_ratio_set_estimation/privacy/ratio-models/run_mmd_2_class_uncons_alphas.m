...
for seed = 1:10
    for test_theta = 0.1:0.1:0.9
        ...
        [mmd_props, alpha_cons] = MMD_uncons(kernel, split, []);
        [mmd_props, alpha_uncons] = MMD_uncons(kernel, split, []);
        fprintf(fid, '%s\t', dataset);
        fprintf(fid, '%s\t', 'CONSvUNCONS');
        fprintf(fid, '%d\t', seed);
        fprintf(fid, '%.2f\t', test_theta);
        for i = 1:2
            fprintf(fid, '%.2f\t', alpha_cons(i));
        end
        for i = 1:2
            fprintf(fid, '%.2f\t', alpha_uncons(i));
        end
        error = norm(alpha_cons - alpha_uncons, 2);
        fprintf(fid, '%.2f\n', error);
    end
end
fclose(fid);