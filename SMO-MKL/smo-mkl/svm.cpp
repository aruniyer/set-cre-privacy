                   ...
                   int d_proj, double lambda_, double obj_threshold_, double diff_threshold_) 
{
  ...
  if(num_kernels>1)
		if(solver_type == SMKL || solver_type == MD)
		{
			...
			switch(d_regularizer)
			{
				...
				case LP:
					if(L_p != 2)
						obj_grad_fp=new obj_grad_lp(d,grad_d,num_kernels,lambda,L_p, L_q);
          else
						obj_grad_fp=new obj_grad_l2(d,grad_d,num_kernels,lambda);
					break;
			}
											 ...
											 proj_c *proj_fp)
{
  ...
  double *proj_grad = new double[num_kernels]; // modified for windows
  ...
  info("g_L2=%f pg_L2=%f Inf=%f Dual=%f gap=%f ", grad_norm_l2, pg_norm_l2, pg_norm_inf, dual, gap);
  delete [] proj_grad;
  return((pg_norm_inf < ProjGrad::inf_abs_tol)||
         (pg_norm_l2 < ProjGrad::l2_abs_tol) ||
         (gap < ProjGrad::kkt_gap_tol) ); 
}
                           ...
                           const double& delta_alpha_j)
{
  ...
  for(n=0;n<num_kernels;n++)
  {
    ...
    if(grad_d[n] > 0)
    {
      // info("BUGBUG: update_Qalpha: grad_d[%d]=%f \n", n, grad_d[n]);
      grad_d[n]=0;
    }
...
int read_kernels(FILE *fp,struct svm_parameter *param,int read_num_kernels)
{
	...
	if(param->num_kernels<=0){
	printf("%d", param->num_kernels);
	 return 2;
}
...
const char *svm_check_parameter(const svm_problem *prob, const svm_parameter *param)
{
  ...
  if(param->solver_type == SMO && param->lambda<=0.0)
  ...
  if(param->d_regularizer == L1 && 
     param->solver_type == SMO)
    return "SMO solver for L1 regularizer not supported yet.";
